...
$JAR_PATH = Join-Path -Path $PWD -ChildPath "..\target\detetive-sql-1.0-SNAPSHOT-jar-with-dependencies.jar"

# Caminho para o diretório de saída
$OUTPUT_DIR = Join-Path -Path $PWD -ChildPath "windows\detective_sql_jre_windows"
...
@echo off
set "appdir=%~dp0"
"%appdir%\bin\java.exe" -Dprism.order=sw -jar "%appdir%\bin\detetive-sql-1.0-SNAPSHOT-jar-with-dependencies.jar"
"@
